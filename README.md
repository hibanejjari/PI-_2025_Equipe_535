...
- `models/*.sql` → create transformation models (cleaning, joining, standardizing)  
- `profiles.yml` → configure PostgreSQL connection details  
- `superset_check.py` → validate Superset data vs reference CSV  
- `superset_fetch.py` → fetch results from Superset API  
- `test_validation.py` → add automated data validation tests  
- `.gitlab-ci.yml` → configure CI/CD pipeline for dbt + Python checks  
