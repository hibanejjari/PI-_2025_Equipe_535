...
# Updated Plan – Dashboard Validation & Anomaly Detection  

### Steps  
1. **Reference Data** – trusted or synthetic KPIs.  
2. **Fetch** – extract dashboard results via Superset API.  
3. **Compare** – align with reference values.  
4. **Classify** – match, mismatch, missing, or outlier.  
5. **Report** – generate CSV/PDF with anomalies.  

### Testing  
- Use anonymized data with injected errors.  
- Focus on aggregated KPIs.  
- Check stability of trends over time.  

### CI/CD  
- Run validation script in GitLab CI/CD.  
- Pipeline fails if anomalies exist.  
- Report stored as artifact for review.  
